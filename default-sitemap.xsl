...
>
	...
	<xsl:template match="/">
		...
		<html xmlns="http://www.w3.org/1999/xhtml">
			<head>
				<title>
					<xsl:choose>
						<xsl:when test="$fileType='Sitemap' or $fileType='RSS'">Mapa del sitio</xsl:when>
		...
		.breadcrumb svg.back {
			...
			<body>
				...
				<xsl:call-template name="Header">
					<xsl:with-param name="title">Mapa del sitio</xsl:with-param>
	...
	<div id="content-head">
		...
		<xsl:choose>
			<xsl:when test="$fileType='RSS'">
				<p>Generado por <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, this is an RSS Sitemap, meant to be consumed by search engines like Google or Bing.</p>
				<p>
			...
			<xsl:otherwise>
				<p>Generado por <a href="https://aioseo.com/?utm_source=WordPress&#038;utm_campaign=liteplugin&#038;utm_medium=xml-sitemap" target="_blank" rel="noreferrer noopener">All in One SEO</a>, esto es un mapa del sitio XML, que se supone consumirán motores de búsqueda como Google o Bing</p>
				<p>
		...
		<xsl:if test="$amountOfURLs &gt; 0">
			<p>
				...
				y se generó el junio 21, 2024 a las 10:08 pm			</p>
		</xsl:if>
	</div>
</xsl:template>
	...
	<xsl:variable name="priorityLabel">
		<xsl:choose>
			<xsl:when test="$priorityLevel = 'low'">Bajo</xsl:when>
			<xsl:when test="$priorityLevel = 'medium'">Medio</xsl:when>
			<xsl:when test="$priorityLevel = 'high'">Alto</xsl:when>
		</xsl:choose>
	</xsl:variable>
